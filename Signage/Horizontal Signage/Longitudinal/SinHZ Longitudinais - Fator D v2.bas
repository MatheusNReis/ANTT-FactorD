...
'SinHZ Longitudinais - Fator D(1).xlsm
...
'MIT License. Copyright © 2024 MatheusNReis
    ...
    MediaSegmento = ThisWorkbook.Sheets("Informações").Cells(7, "F").Value
    ...
    For i = i To LastRowPlanWorks
        ...
        For j = FirstLineReferencia To LastLineReferencia
            If works.Cells(j, MediaSegmento).Value < MinimaRetrorrefletancia Then
                ...
                If InStr(1, works.Cells(FirstLineTrecho, EstacaoMedicao).MergeArea.Cells(1, 1).Value, "+", vbTextCompare) > 0 Then
                    kmEstacao = CDbl(Replace(works.Cells(FirstLineTrecho, EstacaoMedicao).MergeArea.Cells(1, 1).Value, "+", ","))
                Else
                    kmEstacao = CDbl(works.Cells(FirstLineTrecho, EstacaoMedicao).MergeArea.Cells(1, 1).Value)
                End If
                
                ThisWorkbook.Sheets("Compilado").Cells(linhaPlanCompilado, "C").Value = kmEstacao
                ThisWorkbook.Sheets("Compilado").Cells(linhaPlanCompilado, "D").Value = Application.WorksheetFunction.Floor(kmEstacao, ExtSegmento)
                ThisWorkbook.Sheets("Compilado").Cells(linhaPlanCompilado, "E").Value = Application.WorksheetFunction.Floor(kmEstacao, ExtSegmento) + ExtSegmento
                ThisWorkbook.Sheets("Compilado").Cells(linhaPlanCompilado, "F").Value = Rodovia
                ThisWorkbook.Sheets("Compilado").Cells(linhaPlanCompilado, "G").Value = works.Cells(j, FaixaSinalizacao).MergeArea.Cells(1, 1).Value
                ThisWorkbook.Sheets("Compilado").Cells(linhaPlanCompilado, "H").Value = works.Cells(j, MediaSegmento).MergeArea.Cells(1, 1).Value
